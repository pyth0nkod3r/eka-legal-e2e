...
class Conversation(Base):
    ...
    participants = relationship("ConversationParticipant", back_populates="conversation", cascade="all, delete-orphan", lazy="selectin")
    messages = relationship("Message", back_populates="conversation", cascade="all, delete-orphan", lazy="selectin", order_by="Message.timestamp")
...
class Message(Base):
    ...
    id: Mapped[str] = mapped_column(String(50), primary_key=True)
    conversation_id: Mapped[str] = mapped_column(String(50), ForeignKey("conversations.id"), nullable=False, index=True)
    sender_id: Mapped[str] = mapped_column(String(50), ForeignKey("users.id"), nullable=False)
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "senderRole": self.sender_role.value if isinstance(self.sender_role, UserRole) else self.sender_role,
