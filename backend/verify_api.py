...
class APIVerifier:
    ...
    def test_server_connectivity(self) -> bool:
        ...
        try:
            ...
            if resp.status_code == 200:
                self.log_result(
                    "/health", "GET", True, resp.status_code, "Server is healthy"
                )
                return True
    ...
    def test_authentication(self):
        ...
        resp = self.client.post(
            self.api("/auth/login"),
            json={"email": self.user_email, "password": "wrongpassword123"},
        )
        data = resp.json()
        passed = resp.status_code == 200 and data.get("success") is False
        self.log_result(
            "/auth/login (wrong password)", "POST", passed, resp.status_code
        )
        ...
        resp = self.client.post(
            self.api("/auth/forgot-password"), json={"email": self.user_email}
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        resp = self.client.post(
            ...
            json={"token": "fake-token", "password": "newpassword123"},
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        # Test logout
        resp = self.client.post(
            self.api("/auth/logout"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
    ...
    def test_booking_endpoints(self):
        ...
        consult_type_id = (
            consultation_types[0]["id"] if consultation_types else "consult-1"
        )

        # Get available slots
        resp = self.client.get(
            self.api("/booking/available-slots"), params={"date": "2025-01-15"}
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        # Get my bookings
        resp = self.client.get(
            self.api("/booking/bookings"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        if self.booking_id:
            resp = self.client.delete(
                self.api(f"/booking/bookings/{self.booking_id}"),
                headers=self.get_auth_headers(),
            )
            passed = resp.status_code == 200 and resp.json().get("success") is True
            self.log_result(
                "/booking/bookings/{id}", "DELETE", passed, resp.status_code
            )
    ...
    def test_cases_endpoints(self):
        ...
        if self.case_id:
            resp = self.client.get(
                self.api(f"/cases/{self.case_id}"), headers=self.get_auth_headers()
            )
            # May return 403 if case doesn't belong to test user - that's acceptable
            passed = resp.status_code in [200, 403]
            self.log_result("/cases/{id}", "GET", passed, resp.status_code)
        else:
            # Test with non-existent case
            resp = self.client.get(
                self.api("/cases/nonexistent"), headers=self.get_auth_headers()
            )
    ...
    def test_messages_endpoints(self):
        ...
        # Get conversations
        resp = self.client.get(
            self.api("/messages/conversations"), headers=self.get_auth_headers()
        )
        data = resp.json()
        passed = resp.status_code == 200 and data.get("success") is True
        ...
        if self.conversation_id:
            ...
            passed = resp.status_code in [200, 403]
            self.log_result(
                "/messages/conversations/{id}/messages", "GET", passed, resp.status_code
            )
        ...
        passed = resp.status_code == 404
        self.log_result(
            "/messages/conversations/{id}/messages (not found)",
            "GET",
            passed,
            resp.status_code,
            "Expected 404",
    ...
    def test_notifications_endpoints(self):
        ...
        # Get notifications
        resp = self.client.get(
            self.api("/notifications"), headers=self.get_auth_headers()
        )
        data = resp.json()
        passed = resp.status_code == 200 and data.get("success") is True
        ...
        if self.notification_id:
            ...
            self.log_result(
                "/notifications/{id}/read", "POST", passed, resp.status_code
            )

        # Mark all as read
        resp = self.client.post(
            self.api("/notifications/read-all"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
    ...
    def test_dashboard_endpoints(self):
        ...
        # Get client stats
        resp = self.client.get(
            self.api("/dashboard/client/stats"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        resp = self.client.get(
            self.api("/dashboard/lawyer/stats"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
    ...
    def test_intake_endpoints(self):
        ...
        intake_data = {
            ...
            "additionalInfo": {"desiredOutcome": "Resolution", "priorCounsel": "No"},
            "consent": True,
        }
        ...
        # Get intake draft
        resp = self.client.get(
            self.api("/intake/draft"), headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
        ...
        # Save intake draft
        resp = self.client.post(
            self.api("/intake/draft"), json=intake_data, headers=self.get_auth_headers()
        )
        passed = resp.status_code == 200 and resp.json().get("success") is True
    ...
    def print_summary(self) -> bool:
        ...
        if failed > 0:
            ...
            for r in self.results:
                if not r.passed:
                    print(
                        f"   - [{r.method}] {r.endpoint} ({r.status_code}) {r.message}"
                    )
