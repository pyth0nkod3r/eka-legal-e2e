  ...
  return vi.fn((url: string, options?: RequestInit) => {
    ...
    if (endpoint === '/auth/login' && method === 'POST') {
      ...
      if (body.email === 'wrong@example.com') {
        return mockFetchResponse({ success: false, message: 'Invalid email or password' }, 401);
      }
      return mockFetchResponse({
        success: true,
        data: {
          user: { id: 'user-1', email: body.email, name: 'Test User', role: 'client' },
    ...
    if (endpoint === '/auth/register' && method === 'POST') {
      ...
      if (body.email === 'john.doe@email.com') {
        return mockFetchResponse({ success: false, message: 'Email already registered' }, 400);
      }
      return mockFetchResponse({
        success: true,
        data: {
          user: { id: 'new-user', email: body.email, name: body.name, role: 'client' },
          token: 'fake-jwt-token'
        }
      });
    }
    ...
    if (endpoint === '/auth/me' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: { id: 'user-1', email: 'test@example.com', name: 'Test User', role: 'client' }
      });
    }
    ...
    if (endpoint === '/auth/forgot-password' && method === 'POST') {
      return mockFetchResponse({ success: true, message: 'Password reset email sent' });
    }
    ...
    if (endpoint === '/public/lawyer-profile' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: { name: 'Attorney Eka', credentials: ['JD', 'Bar License'], experience: '10 years' }
      });
    }
    ...
    if (endpoint === '/public/services' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [{ id: '1', name: 'Legal Consultation', description: 'Expert advice' }]
      });
    }
    ...
    if (endpoint === '/public/testimonials' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [{ id: '1', name: 'Client', rating: 5, comment: 'Excellent service' }]
      });
    }
    ...
    if (endpoint === '/booking/consultation-types' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [{ id: 'type-1', name: 'Initial Consultation', duration: 60, price: 150 }]
      });
    }
    ...
    if (endpoint === '/booking/bookings' && method === 'POST') {
      ...
      if (body.consultationTypeId === 'invalid-id') {
        return mockFetchResponse({ success: false, message: 'Invalid consultation type' }, 400);
      }
      return mockFetchResponse({
        success: true,
        data: { id: 'booking-1', status: 'pending', clientName: body.name, ...body }
      });
    }
    ...
    if (endpoint === '/booking/bookings' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [
          { id: 'booking-1', clientName: 'Test Client', date: '2024-01-20', time: '10:00' },
          { id: 'booking-2', clientName: 'Another Client', date: '2024-01-21', time: '14:00' }
        ]
      });
    }
    ...
    if (endpoint.match(/^\/cases(\?.*)?$/) && method === 'GET') {
      ...
      const allCases = [
        { id: 'case-1', title: 'Test Case', status: 'active', caseType: 'Corporate', clientId: 'user-1' },
        { id: 'case-2', title: 'Another Case', status: 'pending', caseType: 'Family', clientId: 'user-1' },
        { id: 'case-3', title: 'Closed Case', status: 'closed', caseType: 'Real Estate', clientId: 'user-1' }
      ];
    ...
    if (endpoint.match(/^\/cases\/[^\/]+$/) && method === 'GET') {
      ...
      return mockFetchResponse({
        success: true,
        data: { id: caseId, title: 'Test Case', status: 'active', caseType: 'Corporate', clientId: 'user-1', documents: [] }
      });
    }
    ...
    if (endpoint.match(/^\/cases\/.*\/documents$/) && method === 'POST') {
      return mockFetchResponse({
        success: true,
        data: { id: 'doc-1', name: 'test.pdf', url: '/uploads/test.pdf', uploadedAt: new Date().toISOString() }
      });
    }
    ...
    if (endpoint === '/messages/conversations' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [
          { id: 'conv-1', participants: [{ id: 'user-1', name: 'User' }], lastMessage: 'Hi' },
          { id: 'conv-2', participants: [{ id: 'user-2', name: 'Other' }], lastMessage: 'Hello' }
        ]
      });
    }
    ...
    if (endpoint.match(/^\/messages\/.*\/messages$/) && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [
          { id: 'msg-1', content: 'Hello', senderRole: 'client', timestamp: new Date().toISOString() }
        ]
      });
    }
    ...
    if (endpoint.match(/^\/messages\/.*\/messages$/) && method === 'POST') {
      ...
      return mockFetchResponse({
        success: true,
        data: { id: 'msg-new', content: body.content, senderRole: 'client', timestamp: new Date().toISOString() }
      });
    }
    ...
    if (endpoint === '/notifications' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: [
          { id: 'notif-1', title: 'Notification', message: 'Test', read: false, createdAt: new Date().toISOString() }
        ]
      });
    }
    ...
    if (endpoint === '/dashboard/client/stats' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: { totalClients: 10, activeCase: 5, upcomingAppointments: 3, pendingDocuments: 2 }
      });
    }
    ...
    if (endpoint === '/dashboard/lawyer/stats' && method === 'GET') {
      return mockFetchResponse({
        success: true,
        data: { totalClients: 25, activeCase: 12, upcomingAppointments: 8, pendingDocuments: 5 }
      });
    }
